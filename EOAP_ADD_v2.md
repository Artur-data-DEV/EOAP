...
| Data | 2026-06-06 |
| Versão | 2.0 — Reestruturado por Architecture Review Board |
...
### 11.9 Access Domain Model (User Access Registry)

O ciclo de vida dos direitos de acesso (entitlements) na EOAP é centralizado na tabela `x_eoap_user_access`. A transição de estados segue regras estritas de integridade, determinando quais atores ou automações podem alterar o registro.

#### 11.9.1 Diagrama de Estados do Entitlement
```mermaid
stateDiagram-v2
    [*] --> Requested : Criado via Catalogo / Lifecycle Event
    Requested --> Approved : Aprovado pelo Access Owner / Auto-Approve (DT)
    Requested --> Rejected : Rejeitado pelo Aprovador
    Approved --> Provisioned : Provisionado no target (IAM/OOB Group)
    Provisioned --> Revoked : Revogação Manual / Offboarding / Recertification Fail
    Provisioned --> Expired : Data de expiração (valid_to) atingida
    Revoked --> [*]
    Expired --> [*]
    Rejected --> [*]
```

#### 11.9.2 Atores e Gatilhos por Estado

- **Requested (Solicitado)**:
  - *Criador*: Usuário via Service Portal (Catalog Item), ou Flow de Onboarding/Move (Employee Lifecycle).
  - *Gatilho*: Inserção do registro com dados de validade, justificativa e Business Application associada.
- **Approved (Aprovado)**:
  - *Modificador*: Engine de Aprovação OOB ou Access Owner.
  - *Gatilho*: A Decision Table `DT_EOAP_Access_Approval_Routing` avalia e roteia aprovação. Após aprovação do último nível, o estado muda.
- **Provisioned (Provisionado)**:
  - *Modificador*: Flow Designer Action (outbound REST para IAM) ou Script (adicionando a grupo local).
  - *Gatilho*: Confirmação de provisionamento técnico bem-sucedido.
- **Revoked (Revogado)**:
  - *Modificador*: Flow de Offboarding, tarefa manual do Access Owner ou falha em campanha de recertificação.
  - *Gatilho*: Usuário desligado da empresa, ou revogação manual de acessos incompatíveis após movimentação.
- **Expired (Expirado)**:
  - *Modificador*: Job agendado `EOAP_Job_Access_Expiration`.
  - *Gatilho*: Data `valid_to` é menor que a data atual.
...
| Threat | Vetor | Impacto | Controle |
| --- | --- | --- | --- |
| Privilege Escalation | Usuário atribui-se role administrativo ou access_owner. | Acesso irrestrito a dados e decisões. | ACLs de role assignment, testes negativos ATF, revisão periódica de papéis. |
| Unauthorized Access | Persona sem role tenta ler/escrever registros sensíveis. | Exposição de dados classificados. | Record ACLs + Field ACLs + deny by default. |
| Event Abuse | Publicação de evento malicioso ou duplicado. | Concessão indevida de acesso ou risco incorreto. | Idempotency key, validação de payload, event processing com status. |
| Audit Tampering | Tentativa de editar ou deletar registros de audit trail. | Perda de evidência de compliance. | ACL write-only, deleção proibida, testes negativos ATF. |
| Risk Score Manipulation | Edição manual de x_eoap_risk_score ou x_eoap_risk_band. | Mudança de alto risco aparece como baixo risco. | Field ACL, escrita somente por RiskEngine, evidência em x_eoap_risk_evidence. |
| Self-Approval | Usuário aprova seu próprio pedido de acesso ou exceção. | Violação de SoD. | Exclusão de self-approval em Flow, validação por ATF. |
| Data Exfiltration via Audit | Uso de audit trail para extrair informações sensíveis. | Exposição de payloads. | Field ACL em payload_summary, minimização de dados em logs. |
...
A tabela x_eoap_audit_trail é o repositório aplicacional de evidências. Deve ser tratada como **append-only lógico**:

| Campo | Tipo | Obrigatório | Descrição |
| --- | --- | --- | --- |
| actor | Reference (sys_user) | Sim | Quem realizou a ação. |
| action | Choice | Sim | Tipo de ação (approved, rejected, granted, revoked, calculated, etc.). |
| entity_type | String | Sim | Tipo de entidade (user_access, exception, change, application). |
| entity_sys_id | String | Sim | Sys_id da entidade. |
| before_summary | String | Não | Estado anterior resumido. |
| after_summary | String | Não | Estado posterior resumido. |
| correlation_id | String | Sim | ID de correlação end-to-end. |
| transaction_id | String | Sim | ID de transação ServiceNow. |
| outcome | Choice | Sim | Resultado (success, failure, partial). |
| payload_summary | String | Não | Contexto adicional (dados sensíveis sanitizados). |
| rule_version | String | Não | Versão da regra aplicada (para Risk Engine). |
| created_on | DateTime | Sim | Timestamp de criação (auto). |
...
### 19.1 Assessment Summary

| Dimensão | Nota | Justificativa |
| --- | --- | --- |
| Arquitetura | 8/10 | Arquitetura bem estruturada em camadas com separação de concerns clara. Pontos de melhoria: falta de Logical View formal (C4 Model) e Deployment Architecture detalhada (já corrigido nesta v2). |
| Governança | 9/10 | ADRs abrangentes, RACI definido, governance model claro. Ponto de melhoria: incluir cadência de revisão de ADRs e processo de deprecation formal. |
| Segurança | 8/10 | Threat model definido, SoD clara, ACLs detalhadas por tabela e campo. Pontos de melhoria: incluir controles de MFA para roles sensíveis e política de credential rotation para integrações futuras. |
| Escalabilidade | 7/10 | NFRs definidos com métricas, índices planejados. Pontos de melhoria: falta de capacity planning formal, load testing strategy e horizontal scaling considerations. |
| CSDM | 9/10 | Aderência clara ao modelo, relações documentadas, sem duplicação. Ponto de melhoria: incluir validação formal com ServiceNow CSDM Health Dashboard quando disponível. |
| OOB First | 9/10 | Toda extensão possui ADR com gap documentado. Ponto de melhoria: revisar se x_eoap_event_processing poderia usar Flow Error Handling OOB em cenários mais simples. |
| Operação | 7/10 | Logging padronizado, métricas definidas, event processing. Pontos de melhoria: runbooks formais, escalation matrix e on-call rotation. |
| Observabilidade | 7/10 | Logging, audit trail e métricas definidos. Pontos de melhoria: distributed tracing formal, alerting thresholds e dashboard coverage. |
| Testabilidade | 8/10 | ATF First, suites por pilar, testes negativos de segurança. Ponto de melhoria: incluir test data management strategy e coverage metrics. |

### 19.2 Nota Global

**Score: 8.0/10** — Arquitetura corporativa sólida com maturidade acima da média para ServiceNow platform architecture. Pronta para implementação com as recomendações de melhoria indicadas.

### 19.3 O que Falta para 10/10

| Dimensão | Gap | Recomendação |
| --- | --- | --- |
| Escalabilidade | Capacity planning formal | Definir projeção de volume por tabela para 1, 2 e 5 anos com baseline de PDI. |
| Escalabilidade | Load testing strategy | Criar script de massa sintética e baseline de performance por sprint. |
| Operação | Runbooks formais | Criar runbooks operacionais por pilar com troubleshooting steps. |
| Operação | Escalation matrix | Definir níveis de escalação com SLAs por severidade. |
| Observabilidade | Alerting thresholds | Definir thresholds para cada métrica operacional com ações automatizadas. |
| Observabilidade | Dashboard coverage | Garantir dashboard executivo, operacional e técnico por pilar. |
| Segurança | MFA policy | Definir MFA obrigatório para x_eoap.admin e x_eoap.risk_analyst em produção. |
| Testabilidade | Test data management | Criar estratégia de dados de teste com setup/teardown automatizado por suite. |
| Testabilidade | Coverage metrics | Definir meta de cobertura ATF por artefato (100% critical, 80% high). |
| Arquitetura | C4 Model formal | Criar diagramas C4 (Context, Container, Component) para apresentação ARB. |
...
| Segurança | Roles, ACLs, Field ACLs, SoD, Threat Model. | ✅ Completo |
| Integração | Padrões REST, Import Sets, Correlation ID, Retry. | ✅ Completo |
| Eventos | Catálogo, publishers, consumers, error handling, retry. | ✅ Completo |
| Observabilidade | Logging, métricas, audit trail, monitoring. | ✅ Completo |
